...
The proof of completion is basically copying the contract, compile it via Solang and call it as you
would with the Stellar CLI or the JS SDK.

Alternatively, they can be tested in the playground https://solang.io/. (Not up to date in the time
...
- A current issue of the codebase is the entangled target logic in
  ...
  (Solang's IR emission stage). De-tangling here means that each target should have its own
...
- Improve compiler error reporting: As of now, for the currently not supported Solidity Syntax or
